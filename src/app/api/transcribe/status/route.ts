    ...
    if (job?.TranscriptionJobStatus === 'COMPLETED') {
      ...
      const completion = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: `You are an emoji translator. Convert the given text into a sequence of relevant emojis that represent the meaning of the text. The text is in ${languageCodes.join(', ')}. Use only emojis in your response, no text.`
          },
          {
            role: "user",
            content: transcript
